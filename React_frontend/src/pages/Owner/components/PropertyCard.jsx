...
export default function PropertyCard({ property, onDelete, isDraftMode = false }) {
    ...
    const badgeText = isDraftMode ? 'Draft' : isActive ? 'Available' : 'Unavailable'
                ...
                <div className="absolute right-3 top-3">
                    <span className={`inline-flex items-center rounded-xl px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide backdrop-blur-sm ${badgeClass}`}>
                        <span className={`mr-1.5 h-1.5 w-1.5 rounded-full ${
                            isDraftMode ? 'bg-amber-500' : isActive ? 'bg-emerald-500' : 'bg-slate-400'
                        }`} />
                        {badgeText}
                    </span>
                </div>
            ...
            <div className="flex flex-1 flex-col gap-3 p-4">
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button
                        ...
                        title="Edit"
                        className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 py-2 text-xs font-semibold text-slate-700 transition hover:bg-sky-50 hover:text-sky-600 hover:border-sky-200 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-sky-950/30 dark:hover:text-sky-400 dark:hover:border-sky-800"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center justify-center rounded-xl border border-red-100 p-2 text-red-500 transition hover:bg-red-50 hover:border-red-300 dark:border-red-900/40 dark:text-red-400 dark:hover:bg-red-950/30 dark:hover:border-red-800"
                    >
