...
export default function TenantPayments() {
  ...
  const statCards = [
    {
      label: 'Total paid',
      value: formatAmount(totals.paid, totals.currency),
      icon: CheckCircle2,
    ...
    <div className="space-y-6 sm:space-y-8">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            {loading ? 'Loading payments…' : 'Your payment history across all your bookings.'}
          </p>
        </div>
      ...
      {!loading && !error && payments.length === 0 && (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm dark:border-slate-700 dark:bg-slate-950">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Payments you make for approved bookings will appear here.
          </p>
        </div>
      )}
                    ...
                    <tr key={payment.id} className="transition hover:bg-slate-50/70 dark:hover:bg-slate-900/40">
                      <td className="px-6 py-4">
                        ...
                        {payment.provider_reference && (
                          <p className="font-mono text-[10px] text-slate-400">ref {payment.provider_reference}</p>
                        )}
                      </td>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 font-mono text-xs text-slate-400">{payment.booking_reference}</p>
                  </div>
